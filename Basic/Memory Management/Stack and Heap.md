...
- Không lo lắng quá mức về stack vs heap cho code không nằm trong hot path, vì overhead thường không đáng kể.

## Limitations
- Stack có kích thước giới hạn, mặc định thường `1 MB` trên mỗi thread, gọi đệ quy quá sâu hoặc khai báo quá nhiều local variable lớn sẽ gây `StackOverflowException`.
...
public class Example
{
    public void Run()
    {
        ...
        Enemy enemy = new Enemy(); // heap: object Enemy
                                   // stack: biến reference 'enemy'
        enemy.Name = "Goblin";     // heap: string là reference type
        enemy.Pos = pos;           // copy value lên heap vì Pos là field của class
    }
    // Khi method kết thúc, stack frame bị pop:
    // hp, pos, enemy reference biến mất
    // Object Enemy trên heap chờ GC thu hồi
}
```
