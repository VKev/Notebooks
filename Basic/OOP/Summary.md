...
- `Abstraction`: Giữ lại phần quan trọng của mô hình. Giúp nơi sử dụng bớt phụ thuộc chi tiết cài đặt.
- `Inheritance`: Cho phép class con nhận lại hoặc mở rộng behavior từ class cha. Chỉ nên dùng khi quan hệ cha con thật sự hợp lý.
- `Polymorphism`: Cùng một contract nhưng nhiều implementation. Giúp thay thế object linh hoạt hơn.

## Decision rules
- `Need clear domain model`: Ưu tiên nghĩ theo object và responsibility. Mỗi class nên có vai trò dễ gọi tên.
