...
- `Continuation`: phần code chạy tiếp sau khi việc chờ đã hoàn thành. Chỗ resume quan trọng không kém bản thân việc chờ.
...
- `Awaitable`: Là async type gốc của Unity trong `6.3`, hiểu main thread và update loop tốt hơn. Hợp khi bạn muốn dùng `await` nhưng vẫn bám sát execution model của Unity.

## Popular Unity features
- `Awaitable`: Có các API nổi bật như `NextFrameAsync`, `WaitForSecondsAsync`, `FixedUpdateAsync`, `EndOfFrameAsync`, `MainThreadAsync`, và `BackgroundThreadAsync`. feature trung tâm nếu bạn muốn async flow theo kiểu Unity-native.
- `AsyncOperation`: xuất hiện ở scene loading, resource loading, và các operation do engine xử lý. Có thể theo dõi qua `progress`, `isDone`, event `completed`, hoặc `await` ở các API phù hợp.
...
## Decision rules
- `Need frame timing`: Ưu tiên `Coroutine`. lựa chọn tự nhiên khi flow bám theo player loop.
