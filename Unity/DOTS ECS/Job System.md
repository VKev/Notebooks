...
## When to use it
- Dùng khi system ECS cần xử lý số lượng lớn entity và phần việc có thể chia song song.
- Dùng khi bạn muốn schedule `IJobEntity`, `IJobChunk`, hoặc job song song khác từ `OnUpdate`.

## When to not use it
- Không nên dùng cho logic cực ngắn hoặc quá phụ thuộc main thread API của Unity.
