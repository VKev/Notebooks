...
## One-line
- `Entity` là định danh rất nhẹ trong ECS, dùng để gom các component liên quan lại với nhau mà không tự chứa logic.
...
- Nó giống một `GameObject` rất nhẹ theo nghĩa nhận diện, nhưng bản thân `Entity` không chứa code và không trực tiếp chứa data field như OOP object.
...
- Rất hợp cho số lượng object lớn, nơi cách tổ chức theo `GameObject` và `MonoBehaviour` bắt đầu tốn CPU và cache.

## When to use it
- Dùng khi bạn muốn mô hình hóa số lượng lớn object có cấu trúc dữ liệu rõ ràng.
- Dùng khi object chủ yếu được mô tả bởi data và được nhiều system xử lý tuần tự hoặc song song.

## When to not use it
- Không cần dùng nếu bài toán nhỏ, ít object, hoặc logic phụ thuộc nặng vào workflow `MonoBehaviour`.
- Không phù hợp nếu bạn kỳ vọng mỗi entity là một object giàu behavior như trong OOP.
