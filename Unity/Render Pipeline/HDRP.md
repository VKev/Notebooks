...
## What is it
- Theo tài liệu Unity `6.3 LTS (6000.3)`, `High Definition Render Pipeline` là prebuilt `Scriptable Render Pipeline` high-fidelity, nhắm tới nền tảng hiện đại có hỗ trợ compute shader.
...
- HDRP hỗ trợ cả forward và deferred rendering, trong đó deferred thường hiệu quả hơn cho material phổ biến còn forward phù hợp material phức tạp.
...
## When to use it
- Dùng khi project nhắm tới PC, console, hoặc XR và cần đồ họa cao cấp nhất.
- Dùng khi team có artist cần physical lighting workflow giống production film.
...
- Không dùng nếu project cần chạy trên nhiều nền tảng khác nhau, dùng URP thay thế.
- Không dùng nếu team nhỏ và không cần đồ họa phức tạp, vì learning curve và setup cost cao.
...
// Điều chỉnh HDRP Volume tại runtime
