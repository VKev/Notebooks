...
## What is it
- Theo tài liệu Unity `6.3 LTS (6000.3)`, Scriptable Render Pass là cách thay đổi cách URP render scene hoặc object trong scene.
- Pass thường kế thừa `ScriptableRenderPass` và được inject vào pipeline bằng `ScriptableRendererFeature` hoặc `RenderPipelineManager`.
...
## When to use it
- Dùng khi hiệu ứng cần truy cập camera color/depth hoặc cần chạy đúng thời điểm trong frame.
- Dùng khi built-in Renderer Feature không đủ cho nhu cầu của project.
- Dùng khi cần đóng gói custom rendering thành asset có thể bật/tắt trong Universal Renderer.

## When to not use it
- Không dùng cho logic gameplay hoặc logic không liên quan tới rendering.
- Không dùng nếu shader hoặc material thường đã giải quyết được vấn đề.
