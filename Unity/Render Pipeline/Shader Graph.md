...
## What is it
- Theo tài liệu Unity `6.3 LTS (6000.3)`, `Shader Graph` là visual editor cho phép artist và developer tạo shader bằng cách kéo thả node và nối chúng lại, không cần viết HLSL thủ công.
...
## When to use it
- Dùng khi project dùng `URP` hoặc `HDRP` và cần custom shader.
- Dùng khi artist muốn tạo hiệu ứng visual mà không phụ thuộc vào programmer.
- Dùng cho prototype nhanh hiệu ứng shader trước khi optimize bằng HLSL nếu cần.

## When to not use it
- Không dùng nếu project dùng Built-in RP, vì Shader Graph không hỗ trợ.
