...
## Version scope
- `Unity 6.4 (6000.4)`: Section này chủ yếu dựa trên manual hiện hành. Khuyến nghị thay đổi theo render pipeline, nên luôn đọc đúng pipeline mà project đang dùng.

## Core keywords
- `Draw Call`: Là lúc CPU bảo GPU vẽ thứ gì đó. Chi phí CPU lớn nhất nằm ở render state updates.
- `SetPass calls`: Là chỉ số hữu ích để nhìn lượng chuyển trạng thái render. Unity khuyến nghị kiểm tra trong `Stats`, `Profiler`, hoặc `Frame Debugger`.
...
- `Blend`: Điều khiển cách màu từ fragment shader trộn với render target. Unity `6.4` lưu ý bật blending có thể tắt GPU optimization như `Early-Z`.
- `Cull`: Quy định mặt nào của polygon bị loại bỏ. `Cull Back` là mặc định; `Cull Off` hay dùng cho wall hai mặt hoặc transparent effect.
...
- `ColorMask` và `Offset`: `ColorMask` chặn ghi vào kênh màu; `Offset` tạo depth bias để tránh artifact như z-fighting. Đây cũng là render state nên thay đổi của chúng vẫn ảnh hưởng flow chuẩn bị draw call.
...
- `GPU Instancing`: Một draw call cho nhiều instance cùng mesh và material. hợp cho object lặp lại nhiều lần như cây hoặc prop.
- `SRP Batcher`: Giảm render state updates trong URP/HDRP. Lựa chọn Unity `6.4` khuyến nghị bật trong URP và HDRP.
...
- `Need optimize moving small meshes`: Chỉ thử `Dynamic batching` sau khi profile. Không mặc định coi là tối ưu trên máy hiện đại.

## Common traps
- `Ít draw call hơn luôn tốt hơn`: Sai. Có lúc kỹ thuật giảm draw call lại tăng CPU hoặc memory ở chỗ khác, nên phải profile.
- `GPU Instancing luôn tốt hơn SRP Batcher`: Sai. Trong URP/HDRP, Unity `6.4` khuyến nghị ưu tiên `SRP Batcher` cho case tổng quát.
...
- Trước hết kiểm tra `SRP Batcher`, vì Unity `6.4` khuyến nghị bật cho URP/HDRP trước các lựa chọn cổ điển hơn.

### Khi nào `Dynamic batching` làm chậm hơn?
- Khi chi phí transform vertex trên CPU lớn hơn chi phí của draw call, điều dễ xảy ra trên hardware hiện đại.
